...
DB_PATH       = BASE_DIR / "ersep_ddjj.db"
...
# ══════════════════════════════════════════════════════════════
#  SERVIR FRONTEND (abrí http://localhost:8000 en el navegador)
# ══════════════════════════════════════════════════════════════
...
async def enviar_ddjj(request: Request):
    ...
    # Año: acepta "año", "anio", "aÃo", o cualquier variante unicode corrupta
    anio_raw = None
    for k in form.keys():
        kn = k.lower()
...
def admin_detalle(ddjj_id: int):
    ...
    return {
        ...
        "periodo": f"{MESES[row['mes']]} {row['año']}",
...
def root():
    return {
        "sistema": "ERSeP · DDJJ Transporte Interurbano",
        "version": "2.0",
        "estado": "operativo",
        "docs": "/docs",
    }
